...
# dimensions of the generated pictures for each filter.d
img_width = 48
img_height = 48
...
model = load_model('../models/shallow-357-filt-size-e150')
print('Model loaded.')
...
for filter_index in range(filter_nums):
    ...
    # img = cv2.imread(
    #     '../keras-filter-visualization-master/kb_max_happy701_iter10000_0-9'
    #     '.png')
    # img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    # img = img[:48, :48].reshape((48, 48))
    #
    # input_img_data = img
    # input_img_data = cv2.imread('../images/Happy701.png')
    ...
    # we run gradient ascent for x iterations
    for i in range(iterations):
        loss_value, grads_value = iterate([input_img_data])
        input_img_data += grads_value * step

        # np.reshape(input_img_data*255, (48, 48, 1))
        cv2.imwrite('stitched_filters_temp.png',
                    deprocess_image(input_img_data[0]).reshape((48, 48, 1)))
        
        # print('Current loss value:', loss_value)
        if loss_value <= 0.:
            # some filters get stuck to 0, we can skip them
            print("breaking due to no loss")
            break
